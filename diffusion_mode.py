...
def start():
    ...
    diffusion_game.window = window
    diffusion_game.display = pygame.display.set_mode(
        (width, height))
    diffusion_game.clock = pygame.time.Clock()
    diffusion_game.space = pymunk.Space()
    pygame.init()
    getting_parameters()
    diffusion_game.num_of_balls_1 = num_par1
    diffusion_game.num_of_balls_2 = num_par2
    diffusion_game.el = elasticity
    diffusion_game.speed_limit1 = speed_limit1
    diffusion_game.speed_limit2 = speed_limit2
    diffusion_game.particle_size1 = particle_size1
    diffusion_game.particle_size2 = particle_size2
    diffusion_game.game(width, height)


def poof():
    diffusion_game.poof_wall.delete()


def graphing():
    diffusion_game.nerd_mode = True


def stop():
    diffusion_game.run = False
